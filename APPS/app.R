...
ui <- dashboardPage(
  ...
  header = dashboardHeader(
    ...
    controlbarIcon = NULL,#icon("circle-info"),
    fixed = TRUE
  ),
      ...
      tabItem(
        ...
        fluidRow(
          bs4Card(
            ...
            DTOutput("kpiTablefacebook") %>% withSpinner(),
            width = 12,
            collapsible = TRUE,
            collapsed = FALSE
          )
        ...
        fluidRow(
          box(title = "Facebook Report",
              ...
              status = "olive",
              solidHeader = TRUE, 
              dataTableOutput("facebookTable") %>% withSpinner())
        )
      ),
...
server <- function(input, output) {
  ...
  linkedinData <- reactive({
    read_excel("./data/data.xlsx", sheet = "Linkedin")
  })
  ...
  output$kpiTablelinkedin <- DT::renderDataTable({
    ...
    data <- read_excel("./data/data.xlsx", sheet = "Linkedin")
